...
**Changing the embedding model** — If you edit `embedding_model` in `config.toml`,
existing embeddings in the database were generated by a different model and are
no longer comparable. Run `natalie sync --full` after any embedding-model change
to rebuild the index from scratch.

**Single active session per vault** — `natalie.db` lives inside your vault and
syncs via iCloud like any other file. Running `natalie-server` on two machines
simultaneously against the same synced vault risks SQLite WAL corruption. Keep
